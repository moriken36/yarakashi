...
export default function MainDisplay({ posts }: { posts: Post[] }) {
  ...
  return (
    <div className="flex flex-1 items-center justify-center px-12 -mt-8">
      <div
        className="w-full max-w-5xl rounded-2xl px-16 py-10 shadow-xl transition-opacity duration-500"
      ...
      >
        ...
        {/* Title */}
        {current?.title && (
          <p className="mb-6 text-[1.7rem] font-bold text-gray-500">
            {current.title}
          </p>
        )}

        {/* Body */}
        <p className="whitespace-pre-wrap text-[2.2rem] leading-[1.8] text-gray-800">
          {current?.body}
        </p>
      </div>
    </div>
